 ...
 *
 * <pre>
 * 取中位数分裂，s = floor(m/2) (即向下取整，m为b-tree的阶)，如下图所示：
 ...
 *
 * 以4阶b-tree为例(2<=分支数<=4， 1<=关键码数<=3)演示分裂操作：
 * 将[3]上移至父节点
 *     *[0]*[10]*             *[0]*[3]*[10]*
 *      /  \                     /     \
 *     r    \   s     =>        /       \
 ...
 *
 * <pre>
 * 以4阶b-tree为例(2<=分支数<=4， 1<=关键码数<=3)，当节点关键码数>3时，则该节点发生下溢；
 *
 ...
 *                  /
 *                r+1
 * 若key.search(e)返回r，若key[r]！= e，则有key[r] < e < k[r+1]
 * 故继续向child[r+1]继续查找
 * </pre>
 *
 * @param e: 查找目标
